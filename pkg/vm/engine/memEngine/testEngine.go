...
func CreateR(e engine.Engine) {
	{
...

		{
			attrs = append(attrs, metadata.Attribute{
				Name: "orderId",
				Type: types.Type{Oid: types.TVarchar, Size: 24},
			})
			attrs = append(attrs, metadata.Attribute{
				Name: "uid",
				Type: types.Type{Oid: types.TVarchar, Size: 24},
			...
			attrs = append(attrs, metadata.Attribute{
				Name: "price",
				Type: types.Type{Oid: types.TFloat64, Size: 8, Width: 8},
			})
		}
	...
	}
	{
		...
		{
			{
				vec := vector.New(types.Type{Oid: types.TVarchar, Size: 24})
				vs := make([][]byte, 10)
				for i := 0; i < 10; i++ {
			...
			{
				vec := vector.New(types.Type{types.T(types.TFloat64), 8, 8, 0})
				vs := make([]float64, 10)
				for i := 0; i < 10; i++ {
					vs[i] = float64(i)
				}
	...
	}
	{
		...
		{
			vec := vector.New(types.Type{Oid: types.TFloat64, Size: 8, Width: 8})
			vs := make([]float64, 10)
			for i := 10; i < 20; i++ {
				vs[i-10] = float64(i)
			}
